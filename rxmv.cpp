...
int main(int argc, const char** argv)
{
  ...
  if (argc < 4+skip) {
    ...
    std::cout << "Renames the given list of files.  Each file is compared against the given matchPattern and renamed according to the replaceString.\n\n";
    std::cout << "The matchPattern is any regex accepted by Boost's regex implementation.\n";
    std::cout << "The replaceString gives the new filename's format.  Matches from the matchPattern are substituted according to the rules of Boost's format implementation.\n\n";
    std::cout << "Renaming only occurs if the '-x' flag is passed in.  Otherwise, a report is printed showing what the results of the renaming would be.\n"; 
